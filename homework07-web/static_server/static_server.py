...
class StaticHTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def handle_request(self, request: HTTPRequest) -> HTTPResponse:
        ...
        if request.method == b"HEAD":
            content = b""
        elif request.method == b"GET" or request.method == b"HEAD":
            status = 200
            ...
            if os.path.exists(path) and os.path.isfile(path):
                try:
                    ...
                    print(f"Ooops... Error 404")
                    status = 404
                except Exception:
                    print(f"Ooops... Error 500")
                    status = 500
            elif not os.path.exists(path):
                status = 404
                print("Ooops... File not found. Error 404")
            elif os.path.exists(path) and not os.path.isfile(path):
                status = 404
                print("Ooops... It is not a file. Error 404")
